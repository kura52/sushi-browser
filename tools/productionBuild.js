...
function build(){
  ...
  if (isWindows) {
    ...
    fs.writeFileSync(`${pwd}/${buildDir}/version`, 'v4.0.0')
    ;[`node ./node_modules/.bin/electron-installer-debian --src ${buildDir}/ --dest ${outDir}/ --arch amd64 --config res/linuxPackaging.json`,
      `node ./node_modules/.bin/electron-installer-redhat --src ${buildDir}/ --dest ${outDir}/ --arch x86_64 --config res/linuxPackaging.json`,
