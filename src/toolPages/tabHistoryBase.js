...
function getHistory(name){
  ...
  return new Promise((resolve,reject)=>{
    ipc.send('fetch-history',cond,true)
...
async function buildItem(h,nodePath,dirc=[]){
  ...
  for(let i=len-1;i>=0;i--){
    ...
    const ele = {
      name: `[${h.updated_at.slice(11,16)}][${len - i}] ${name && name.length > 55 ? `${name.substr(0, 55)}...` : name}`,
      url: convertURL(url),
      _url: url,
  ...
  dirc[dirc.length -1].divider = true
  return dirc
}
