...
export default class NavbarMenu extends Component {
  ...
  componentDidUpdate(prevProps, prevState){
    ...
    if((!this.props.mouseOver || this.props.left) && !this.props.rightDisplay){
      const func = _=>{
  ...
  render(){
    ...
    return <div onContextMenu={this.props.onContextMenu} onMouseOver={this.props.mouseOver ? this.onMouseOver : (void 0)}
                ...
                aria-expanded="false" className={`${this.props.className != 'main-menu' ? 'draggable-source' : ''} ui top${(this.props.mouseOver && !this.props.left) || this.props.rightDisplay ? '' : ' right pointing'} nav-button dropdown ${this.props.className}`}
      ...
      {this.props.badge ? <div className="browserActionBadge video-controller" >{this.props.badge()}</div> : null}
      {!this.state.visible && !this.state.forceOpen ? null :
        <div key="menu" ref="menu" style={{...styleMenu,...this.props.style}}
             className={`menu${this.state.visible || this.state.forceOpen ? " visible" : ""} transition left ${this.props.audio ? 'nav-menu-audio' : this.props.rightDisplay ? 'nav3-menu' : (this.props.mouseOver && !this.props.left)  ? 'nav2-menu' : 'nav-menu'}`} >
        {(list || []).map((child) => {
