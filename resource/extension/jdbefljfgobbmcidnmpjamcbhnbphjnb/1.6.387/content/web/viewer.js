...
var pdfjsWebLibs;
{
 ...
 (function () {
  ...
  (function (root, factory) {
   factory(root.pdfjsWebChromeCom = {}, root.pdfjsWebApp, root.pdfjsWebOverlayManager, root.pdfjsWebPreferences, root.pdfjsWebPDFJS);
    ...
    if (/^file?:/.test(file)) {
     getEmbedderOrigin(function (origin) {
      if (origin && !/^file:|^chrome-extension:/.test(origin)) {
       PDFViewerApplication.error('Blocked ' + origin + ' from loading ' + file + '. Refused to load a local file in a non-local page ' + 'for security reasons.');
       return;
      }
